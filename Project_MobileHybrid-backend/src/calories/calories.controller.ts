...
export class CaloriesController {
  ...
  async searchFoods(
    ...
    const foods = await this.foodSearchService.searchFoods(query.trim(), 10, isBranded);
    return {
      success: true,
      data: foods,
    };
  ...
  async logMeal(@CurrentUser() user: AuthUser, @Body() logMealDto: LogMealDto) {
  ...
  @Put('target')
  @UseGuards(JwtAuthGuard)
  async setDailyTarget(@CurrentUser() user: AuthUser, @Body() targetDto: DailyCalorieTargetDto) {
    return this.caloriesService.setDailyTarget(user.id, targetDto.dailyTarget);
  }
  @Get('daily-stats/me')
  @UseGuards(JwtAuthGuard)
  async getMyDailyStats(@CurrentUser() user: AuthUser) {
  ...
  @UseGuards(JwtAuthGuard)
  async getDailyStats(@CurrentUser() user: AuthUser, @Param('userId') userId: string) {
  ...
  @Get('history/me')
  @UseGuards(JwtAuthGuard)
  async getMyCalorieHistory(@CurrentUser() user: AuthUser) {
  ...
  @Get('stats/me')
  @UseGuards(JwtAuthGuard)
  async getMyTotalStats(@CurrentUser() user: AuthUser) {
  ...
  @Get('stats/:userId')
  @UseGuards(JwtAuthGuard)
  async getTotalStats(@CurrentUser() user: AuthUser, @Param('userId') userId: string) {
  ...
  @Delete('meals/:id')
  @UseGuards(JwtAuthGuard)
  async deleteMeal(@CurrentUser() user: AuthUser, @Param('id') id: string) {
